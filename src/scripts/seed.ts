...
import { EducationExperienceList, Experience, WorkExperienceList } from "@/types";
import { datoCMS } from "@services/datoCMS";
import { educationExperienceQuery, queryWrapper, workExperienceQuery } from "@/lib/queries";
...
async function seed() {
    ...
    try {
        const [educationExperiences, workExperiences] = await Promise.all([
            datoCMS({ query: queryWrapper([educationExperienceQuery]) }) as Promise<EducationExperienceList>,
            datoCMS({ query: queryWrapper([workExperienceQuery]) }) as Promise<WorkExperienceList>
        ]);
 ...
 */
const getExperienceSeedData = ({ type, experience }: { type: string, experience: Experience }) => {
    
    const cleanContent = experience.content.replace(/\* /g, '').trim();

    // Create a semantically rich paragraph
